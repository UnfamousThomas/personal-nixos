#!/usr/bin/env bash
# Adapted from https://github.com/tirtha4/Granola-for-Linux (granola-linux.sh).
# Only change: CACHE_DIR defaults to ~/.cache/granola instead of next to the
# script, since here the script lives in the read-only Nix store.

set -euo pipefail

DMG="${1:-}"
INSTALL_DIR="${INSTALL_DIR:-$HOME/Applications/granola}"
CACHE_DIR="${CACHE_DIR:-${XDG_CACHE_HOME:-$HOME/.cache}/granola}"
...
[[ -n "$DMG" ]] || die "usage: granola-install <path-to-granola.dmg>   (INSTALL_DIR=$INSTALL_DIR)"
